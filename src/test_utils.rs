...
/// Translate from a String MiniscriptKey type to groestlcoin::PublicKey
...
pub struct StrKeyTranslator {
    pub pk_map: HashMap<String, groestlcoin::PublicKey>,
...
impl Translator<String, groestlcoin::PublicKey, ()> for StrKeyTranslator {
...
/// Same as [`StrKeyTranslator`], but for [`groestlcoin::XOnlyPublicKey`]
#[derive(Debug, PartialEq, Eq, Clone)]
pub struct StrXOnlyKeyTranslator {
    pub pk_map: HashMap<String, groestlcoin::XOnlyPublicKey>,
    pub pkh_map: HashMap<String, hash160::Hash>,
    pub sha256_map: HashMap<String, sha256::Hash>,
    pub ripemd160_map: HashMap<String, ripemd160::Hash>,
    pub hash160_map: HashMap<String, hash160::Hash>,
...
impl Translator<String, groestlcoin::XOnlyPublicKey, ()> for StrXOnlyKeyTranslator {
    fn pk(&mut self, pk: &String) -> Result<groestlcoin::XOnlyPublicKey, ()> {
        let key = self.pk_map.get(pk).copied().unwrap_or_else(|| {
            groestlcoin::XOnlyPublicKey::from_str(
...
impl StrKeyTranslator {
    pub fn new() -> Self {
        ...
        let pks: Vec<_> = sks
            .iter()
            .map(|sk| groestlcoin::PublicKey::new(secp256k1::PublicKey::from_secret_key(&secp, sk)))
            .collect();
    ...
    pub fn new() -> Self {
        ...
        let pks: Vec<_> = sks
            ...
            .map(|sk| {
                ...
                let (pk, _parity) = groestlcoin::XOnlyPublicKey::from_keypair(&keypair);
                pk
            })
            .collect();
        let mut pk_map = HashMap::new();
